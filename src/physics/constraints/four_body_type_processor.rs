...
pub struct FourBodyTypeProcessorImpl<
    ...
    TWarmStartAccessFilterC: IBodyAccessFilter + 'static,
    TWarmStartAccessFilterD: IBodyAccessFilter + 'static,
...
> {
    ...
    _marker: PhantomData<(
        ...
        TWarmStartAccessFilterC,
        TWarmStartAccessFilterD,
        TSolveAccessFilterA,
        TSolveAccessFilterB,
        TSolveAccessFilterC,
        TSolveAccessFilterD,
    ...
    >
{
    ...
    fn get_bundle_type_sizes(
        ...
        *body_references_bundle_size = std::mem::size_of::<FourBodyReferences>() as i32;
    ...
    fn generate_sort_keys_and_copy_references(
        ...
        unsafe {
            let body_references = type_batch.body_references.as_ptr() as *const FourBodyReferences;
            for i in 0..constraint_count {
                *first_source_index.add(i as usize) = local_constraint_start + i;
                ...
                let d = refs.index_d[inner_index];
                *first_sort_key.add(i as usize) = a.min(b).min(c.min(d));
            }
            // Copy body references to cache
            let ref_size = std::mem::size_of::<FourBodyReferences>();
            let src = type_batch
                .body_references
                .as_ptr()
    ...
    fn regather(
        ...
        unsafe {
            let body_ref_cache = body_references_cache.as_ptr() as *const FourBodyReferences;
            ...
            let body_ref_target =
                type_batch.body_references.as_mut_ptr() as *mut FourBodyReferences;
            ...
            for i in 0..constraint_count {
                ...
                dst_ref.index_d[target_inner] = src_ref.index_d[source_inner];
        ...
        unsafe {
            ...
            for i in start_bundle..exclusive_end_bundle {
                ...
                if batch_integration_mode == crate::physics::constraints::batch_integration_mode::BatchIntegrationMode::Never {
                    ...
                    bodies.scatter_velocities::<TWarmStartAccessFilterC>(&wsv_c, &references.index_c);
                    bodies.scatter_velocities::<TWarmStartAccessFilterD>(&wsv_d, &references.index_d);
                } else {
        ...
        unsafe {
            ...
            for i in start_index..end_index {
                ...
                *target_type_batch.index_to_handle.get_mut(i) = source_handle;
                ...
                dst_refs.index_c[target_inner] = active_body_set
                    .index_to_handle
                    .get(encoded_index_c & Bodies::BODY_REFERENCE_MASK)
                    .0
                    | (encoded_index_c & (Bodies::KINEMATIC_MASK as i32));
                dst_refs.index_d[target_inner] = active_body_set
                    .index_to_handle
                    .get(encoded_index_d & Bodies::BODY_REFERENCE_MASK)
                    .0
                    | (encoded_index_d & (Bodies::KINEMATIC_MASK as i32));
            }
        }
    }
    ...
    fn copy_sleeping_to_active(
        ...
        unsafe {
            ...
            for i in 0..count {
                ...
                let encoded_handle_c = src_refs.index_c[source_inner];
                let encoded_handle_d = src_refs.index_d[source_inner];
                ...
                dst_refs.index_c[target_inner] = bodies
                    .handle_to_location
                    .get(encoded_handle_c & Bodies::BODY_REFERENCE_MASK)
                    .index
                    | (encoded_handle_c & (Bodies::KINEMATIC_MASK as i32));
                dst_refs.index_d[target_inner] = bodies
                    .handle_to_location
                    .get(encoded_handle_d & Bodies::BODY_REFERENCE_MASK)
                    .index
                    | (encoded_handle_d & (Bodies::KINEMATIC_MASK as i32));
        ...
        unsafe {
            ...
            for bundle_index_in_source in 0..source_bundle_count {
                ...
                for source_inner_index in 0..count_in_bundle {
                    ...
                    let encoded_handle_c = src_refs.index_c[source_inner_index as usize];
                    let encoded_handle_d = src_refs.index_d[source_inner_index as usize];
                    ...
                    let body_handle_c = encoded_handle_c & Bodies::BODY_REFERENCE_MASK;
                    let body_handle_d = encoded_handle_d & Bodies::BODY_REFERENCE_MASK;
                    let body_indices = [
                        bodies.handle_to_location.get(body_handle_a).index
                            | (encoded_handle_a & (Bodies::KINEMATIC_MASK as i32)),
                        bodies.handle_to_location.get(body_handle_b).index
                            | (encoded_handle_b & (Bodies::KINEMATIC_MASK as i32)),
                        bodies.handle_to_location.get(body_handle_c).index
                            | (encoded_handle_c & (Bodies::KINEMATIC_MASK as i32)),
                        bodies.handle_to_location.get(body_handle_d).index
                            | (encoded_handle_d & (Bodies::KINEMATIC_MASK as i32)),
                    ];
    ...
    fn add_waking_body_handles_to_batch_references(
        ...
        unsafe {
            let body_refs = type_batch.body_references.as_ptr() as *const FourBodyReferences;
            for i in 0..type_batch.constraint_count {
                ...
                let encoded_c = refs.index_c[inner_index];
                if Bodies::is_encoded_dynamic_reference(encoded_c) {
                    target_batch_referenced_handles.set_unsafely(encoded_c);
                }
                let encoded_d = refs.index_d[inner_index];
                if Bodies::is_encoded_dynamic_reference(encoded_d) {
                    target_batch_referenced_handles.set_unsafely(encoded_d);
                }
            }
        }
    }
        ...
        unsafe {
            ...
            for bundle_index in 0..bundle_count {
                ...
                for inner_index in 0..bundle_size {
                    ...
                    if refs.index_c[inner_index] == body_to_find {
                        count += 1;
                    }
                    if refs.index_d[inner_index] == body_to_find {
                        count += 1;
                    }
                }
            }
