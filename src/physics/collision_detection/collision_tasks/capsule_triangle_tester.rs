...
use crate::physics::collision_detection::mesh_reduction::{FACE_COLLISION_FLAG, MINIMUM_DOT_FOR_FACE_COLLISION};
...
impl CapsuleTriangleTester {
    ...
    pub fn test_edge(
        ...
        Vector3Wide::scale_to(edge_offset, &(Vector::<f32>::splat(1.0) / edge_length), edge_direction);
        ...
        *tb = *ta * dadb - db_offset_b;
        ...
        let ta1 = (*capsule_half_length).simd_min((-*capsule_half_length).simd_max(da_offset_b + edge_length * dadb));
        let a_min = ta0.simd_min(ta1);
        let a_max = ta0.simd_max(ta1);
        let a_onto_b_offset = *capsule_half_length * dadb.abs();
        *b_min = Vector::<f32>::splat(0.0).simd_max(edge_length.simd_min(-a_onto_b_offset - db_offset_b));
        *b_max = edge_length.simd_min(Vector::<f32>::splat(0.0).simd_max(a_onto_b_offset - db_offset_b));
        ...
        *tb = (*tb).simd_min(*b_max).simd_max(*b_min);
        ...
        unsafe { Vector3Wide::cross_without_overlap(capsule_axis, edge_offset, &mut fallback_normal) };
        let mut calibration_dot = Vector::<f32>::splat(0.0);
        Vector3Wide::dot(&fallback_normal, capsule_center, &mut calibration_dot);
        Vector3Wide::conditionally_negate(&calibration_dot.simd_lt(Vector::<f32>::splat(0.0)).to_int(), &mut fallback_normal);
        ...
        *normal = Vector3Wide::conditional_select(&use_fallback_normal.to_int(), &fallback_normal, normal);
        normal_length_squared = use_fallback_normal.select(fallback_normal_length_squared, normal_length_squared);

        // Second fallback: edge plane normal (triangle normal x edge).
        let mut second_fallback_normal = Vector3Wide::default();
        unsafe { Vector3Wide::cross_without_overlap(triangle_normal, edge_offset, &mut second_fallback_normal) };
        ...
        *normal = Vector3Wide::conditional_select(&use_second_fallback.to_int(), &second_fallback_normal, normal);
        normal_length_squared = use_second_fallback.select(second_fallback_length_squared, normal_length_squared);

        *normal = Vector3Wide::scale(normal, &(Vector::<f32>::splat(1.0) / StdFloat::sqrt(normal_length_squared)));
    ...
    pub fn clip_against_edge_plane(
        ...
        unsafe { Vector3Wide::cross_without_overlap(face_normal, edge_offset, &mut edge_plane_normal) };
        ...
        let t_val = velocity_is_positive.select(-distance, distance) / velocity.abs().simd_max(Vector::<f32>::splat(1e-15));
    ...
    pub fn test(
        ...
        local_triangle_center = Vector3Wide::scale(&local_triangle_center, &Vector::<f32>::splat(1.0 / 3.0));
        ...
        Matrix3x3Wide::transform_by_transposed_without_overlap(&world_capsule_axis, &r_b, &mut local_capsule_axis);
        ...
        let mut face_normal = Vector3Wide::scale(&acxab, &(Vector::<f32>::splat(1.0) / face_normal_length));
        ...
        Vector3Wide::dot(&face_normal, &local_offset_a, &mut capsule_offset_along_normal);
        ...
        Self::test_edge(&triangle, &face_normal, &triangle.a, &ab, &local_offset_a, &local_capsule_axis, &a.half_length,
            &mut edge_direction, &mut ta, &mut tb, &mut b_min_val, &mut b_max_val, &mut edge_depth, &mut edge_normal);
        ...
        Self::test_edge(&triangle, &face_normal, &triangle.a, &ac, &local_offset_a, &local_capsule_axis, &a.half_length,
            &mut edge_direction_cand, &mut ta_cand, &mut tb_cand, &mut b_min_cand, &mut b_max_cand, &mut edge_depth_cand, &mut edge_normal_cand);
        let use_ac = edge_depth_cand.simd_lt(edge_depth);
        edge_direction = Vector3Wide::conditional_select(&use_ac.to_int(), &edge_direction_cand, &edge_direction);
        edge_normal = Vector3Wide::conditional_select(&use_ac.to_int(), &edge_normal_cand, &edge_normal);
        ...
        Self::test_edge(&triangle, &face_normal, &triangle.b, &bc, &local_offset_a, &local_capsule_axis, &a.half_length,
            &mut edge_direction_cand, &mut ta_cand, &mut tb_cand, &mut b_min_cand, &mut b_max_cand, &mut edge_depth_cand, &mut edge_normal_cand);
        let use_bc = edge_depth_cand.simd_lt(edge_depth);
        let edge_start = Vector3Wide::conditional_select(&use_bc.to_int(), &triangle.b, &triangle.a);
        edge_direction = Vector3Wide::conditional_select(&use_bc.to_int(), &edge_direction_cand, &edge_direction);
        edge_normal = Vector3Wide::conditional_select(&use_bc.to_int(), &edge_normal_cand, &edge_normal);
        ...
        let mut local_normal = Vector3Wide::conditional_select(&use_edge.to_int(), &edge_normal, &face_normal);
        let mut local_normal_dot_face_normal = Vector::<f32>::splat(0.0);
        Vector3Wide::dot(&local_normal, &face_normal, &mut local_normal_dot_face_normal);
        let colliding_with_solid_side = local_normal_dot_face_normal.simd_ge(Vector::<f32>::splat(TriangleWide::BACKFACE_NORMAL_DOT_REJECTION_THRESHOLD));
        ...
        TriangleWide::compute_nondegenerate_triangle_mask(&ab_length_sq, &ca_length_sq, &face_normal_length, &mut _epsilon_scale, &mut nondegenerate_mask);
        let negative_margin = -*speculative_margin;
        let allow_contacts = (depth + a.radius).simd_ge(negative_margin).to_int() & active_lanes & colliding_with_solid_side.to_int() & nondegenerate_mask;
        ...
        if use_edge_masked.simd_lt(Vector::<i32>::splat(0)).any() {
            ...
            unsafe { Vector3Wide::cross_without_overlap(&edge_direction, &edge_normal, &mut plane_normal) };
            ...
            let squared_angle = plane_normal_length_squared.simd_lt(Vector::<f32>::splat(1e-10)).select(
            ...
            contact_count = use_edge_masked.simd_lt(Vector::<i32>::splat(0)).select(
                b_max_weighted.simd_gt(b_min_weighted).select(Vector::<i32>::splat(2), Vector::<i32>::splat(1)),
                Vector::<i32>::splat(0),
            );
        } else {
        ...
        if ((contact_count.simd_le(Vector::<i32>::splat(1)).to_int()) & allow_contacts).simd_lt(Vector::<i32>::splat(0)).any() {
            let mut ab_entry = Vector::<f32>::splat(0.0);
            let mut ab_exit = Vector::<f32>::splat(0.0);
            Self::clip_against_edge_plane(&triangle.a, &ab, &face_normal, &local_offset_a, &local_capsule_axis, &mut ab_entry, &mut ab_exit);
            let mut bc_entry = Vector::<f32>::splat(0.0);
            let mut bc_exit = Vector::<f32>::splat(0.0);
            Self::clip_against_edge_plane(&triangle.b, &bc, &face_normal, &local_offset_a, &local_capsule_axis, &mut bc_entry, &mut bc_exit);
            ...
            Self::clip_against_edge_plane(&triangle.a, &ca_local, &face_normal, &local_offset_a, &local_capsule_axis, &mut ca_entry, &mut ca_exit);
            ...
            let interval_is_valid = overlap_interval_max_clamped.simd_ge(overlap_interval_min_clamped);
            let overlap_interval_min_final = overlap_interval_min_clamped.simd_min(a.half_length);
            let overlap_interval_max_final = overlap_interval_max_clamped.simd_max(negative_half_length);

            let mut clipped_on_a0 = Vector3Wide::scale(&local_capsule_axis, &overlap_interval_min_final);
            ...
            let mut clipped_on_a1 = Vector3Wide::scale(&local_capsule_axis, &overlap_interval_max_final);
            ...
            let allow_face_contacts = capsule_offset_along_normal.simd_ge(Vector::<f32>::splat(0.0));
            ...
            contact_count = use_face_contacts.simd_lt(Vector::<i32>::splat(0)).select(Vector::<i32>::splat(2), contact_count);

            // Second contact from face if only 1 edge contact.
            let use_face1_for_second = (overlap_interval_max_final - ta).abs().simd_gt((overlap_interval_min_final - ta).abs());
            let second_contact_candidate = Vector3Wide::conditional_select(&use_face1_for_second.to_int(), &face_candidate1, &face_candidate0);
            let second_contact_distance = use_face1_for_second.select(distance_along_normal_a1, distance_along_normal_a0);
            let use_candidate_for_second = interval_is_valid.to_int()
                & contact_count.simd_eq(Vector::<i32>::splat(1)).to_int()
                & second_contact_distance.simd_gt(Vector::<f32>::splat(0.0)).to_int();
            b1 = Vector3Wide::conditional_select(&use_candidate_for_second, &second_contact_candidate, &b1);
            contact_count = use_candidate_for_second.simd_lt(Vector::<i32>::splat(0)).select(Vector::<i32>::splat(2), contact_count);
        }

        // Per-contact depth via capsule face normal projection.
        let mut capsule_tangent = Vector3Wide::default();
        unsafe { Vector3Wide::cross_without_overlap(&local_normal, &local_capsule_axis, &mut capsule_tangent) };
        let mut face_normal_a = Vector3Wide::default();
        unsafe { Vector3Wide::cross_without_overlap(&capsule_tangent, &local_capsule_axis, &mut face_normal_a) };
        ...
        manifold.contact0_exists = allow_contacts & contact_count.simd_gt(Vector::<i32>::splat(0)).to_int() & manifold.depth0.simd_gt(negative_margin).to_int();
        manifold.contact1_exists = allow_contacts & (contact_count.simd_eq(Vector::<i32>::splat(2)).to_int() & (!collapse.to_int())) & manifold.depth1.simd_gt(negative_margin).to_int();
        ...
        manifold.feature_id0 = flip_feature_ids.select(Vector::<i32>::splat(1), Vector::<i32>::splat(0));
        manifold.feature_id1 = flip_feature_ids.select(Vector::<i32>::splat(0), Vector::<i32>::splat(1));

        let face_flag = local_normal_dot_face_normal.simd_ge(Vector::<f32>::splat(MINIMUM_DOT_FOR_FACE_COLLISION))
