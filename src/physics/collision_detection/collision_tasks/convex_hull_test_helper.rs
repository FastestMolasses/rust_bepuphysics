...
impl ConvexHullTestHelper {
    ...
    pub fn pick_representative_face(
        ...
        Vector3Wide::dot(&first_plane.normal, &slot_local_normal_bundle, &mut best_face_dot_bundle);
        let mut closest_on_hull_dot = Vector::<f32>::splat(0.0);
        Vector3Wide::dot(&first_plane.normal, &slot_closest_on_hull, &mut closest_on_hull_dot);
        ...
        for i in 1..hull.bounding_planes.len() as usize {
            let slot_indices = Vector::<i32>::splat((i << vector_shift) as i32) + *slot_offset_indices;
            ...
            Vector3Wide::dot(&bounding_plane.normal, &slot_closest_on_hull, &mut candidate_dot_hull);
            ...
            best_plane_error_bundle = use_candidate.select(candidate_error, best_plane_error_bundle);
            best_indices = use_candidate_i.simd_ne(Vector::<i32>::splat(0)).select(slot_indices, best_indices);
        }
        ...
        *best_face_index = best_indices[0];
        ...
        BundleIndexing::get_bundle_indices(*best_face_index as usize, &mut face_bundle_index, &mut face_inner_index);
