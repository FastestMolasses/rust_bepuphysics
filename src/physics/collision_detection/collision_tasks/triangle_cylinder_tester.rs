...
impl TriangleCylinderTester {
    ...
    pub unsafe fn test(
        ...
        <CylinderSupportFinder as ISupportFinder<CylinderWide>>::compute_local_support(
            ...
            &inactive_lanes,
            &mut cylinder_support_along_negated_tn,
        );
        let mut negated_tn_support = Vector3Wide::default();
        Vector3Wide::subtract(
            &cylinder_support_along_negated_tn,
            &local_triangle_center,
            &mut negated_tn_support,
        );
        ...
        if use_cap_i.simd_lt(zero_i).any() {
            ...
            Vector3Wide::scale_to(
                &triangle_ab,
                &(one_f / triangle_ab_length),
                &mut triangle_tangent_x,
            );
            ...
            if use_cap_triangle_face_i.simd_lt(zero_i).any() {
                ...
                BoxCylinderTester::generate_interior_points(
                    ...
                    &local_normal,
                    &closest_on_b,
                    &mut interior_on_cyl0,
                    &mut interior_on_cyl1,
                    &mut interior_on_cyl2,
                    &mut interior_on_cyl3,
                );
                ...
                let t0 = (x_offset0 * local_normal.x
                    + y_offset * local_normal.y
                    + z_offset0 * local_normal.z)
                    * inverse_denominator;
                let t1 = (x_offset1 * local_normal.x
                    + y_offset * local_normal.y
                    + z_offset1 * local_normal.z)
                    * inverse_denominator;
                let t2 = (x_offset2 * local_normal.x
                    + y_offset * local_normal.y
                    + z_offset2 * local_normal.z)
                    * inverse_denominator;
                let t3 = (x_offset3 * local_normal.x
                    + y_offset * local_normal.y
                    + z_offset3 * local_normal.z)
                    * inverse_denominator;
                ...
                Vector3Wide::dot(
                    &local_normal,
                    &triangle_tangent_x,
                    &mut tangent_local_normal.x,
                );
                Vector3Wide::dot(
                    &local_normal,
                    &triangle_tangent_y,
                    &mut tangent_local_normal.y,
                );
                ...
                let interior0 = Vector2Wide {
                    x: tangent_local_normal.x * t0
                        - x_offset0 * triangle_tangent_x.x
                        - y_on_tangent_x
                        - z_offset0 * triangle_tangent_x.z,
                    y: tangent_local_normal.y * t0
                        - x_offset0 * triangle_tangent_y.x
                        - y_on_tangent_y
                        - z_offset0 * triangle_tangent_y.z,
                };
                let interior1 = Vector2Wide {
                    x: tangent_local_normal.x * t1
                        - x_offset1 * triangle_tangent_x.x
                        - y_on_tangent_x
                        - z_offset1 * triangle_tangent_x.z,
                    y: tangent_local_normal.y * t1
                        - x_offset1 * triangle_tangent_y.x
                        - y_on_tangent_y
                        - z_offset1 * triangle_tangent_y.z,
                };
                let interior2 = Vector2Wide {
                    x: tangent_local_normal.x * t2
                        - x_offset2 * triangle_tangent_x.x
                        - y_on_tangent_x
                        - z_offset2 * triangle_tangent_x.z,
                    y: tangent_local_normal.y * t2
                        - x_offset2 * triangle_tangent_y.x
                        - y_on_tangent_y
                        - z_offset2 * triangle_tangent_y.z,
                };
                let interior3 = Vector2Wide {
                    x: tangent_local_normal.x * t3
                        - x_offset3 * triangle_tangent_x.x
                        - y_on_tangent_x
                        - z_offset3 * triangle_tangent_x.z,
                    y: tangent_local_normal.y * t3
                        - x_offset3 * triangle_tangent_y.x
                        - y_on_tangent_y
                        - z_offset3 * triangle_tangent_y.z,
                };
            ...
            local_offset_b0.x = triangle_tangent_x.x * candidate0.x
                + triangle_tangent_y.x * candidate0.y
                + local_triangle_center.x;
            local_offset_b0.y = triangle_tangent_x.y * candidate0.x
                + triangle_tangent_y.y * candidate0.y
                + local_triangle_center.y;
            local_offset_b0.z = triangle_tangent_x.z * candidate0.x
                + triangle_tangent_y.z * candidate0.y
                + local_triangle_center.z;
            local_offset_b1.x = triangle_tangent_x.x * candidate1.x
                + triangle_tangent_y.x * candidate1.y
                + local_triangle_center.x;
            local_offset_b1.y = triangle_tangent_x.y * candidate1.x
                + triangle_tangent_y.y * candidate1.y
                + local_triangle_center.y;
            local_offset_b1.z = triangle_tangent_x.z * candidate1.x
                + triangle_tangent_y.z * candidate1.y
                + local_triangle_center.z;
            local_offset_b2.x = triangle_tangent_x.x * candidate2.x
                + triangle_tangent_y.x * candidate2.y
                + local_triangle_center.x;
            local_offset_b2.y = triangle_tangent_x.y * candidate2.x
                + triangle_tangent_y.y * candidate2.y
                + local_triangle_center.y;
            local_offset_b2.z = triangle_tangent_x.z * candidate2.x
                + triangle_tangent_y.z * candidate2.y
                + local_triangle_center.z;
            local_offset_b3.x = triangle_tangent_x.x * candidate3.x
                + triangle_tangent_y.x * candidate3.y
                + local_triangle_center.x;
            local_offset_b3.y = triangle_tangent_x.y * candidate3.x
                + triangle_tangent_y.y * candidate3.y
                + local_triangle_center.y;
            local_offset_b3.z = triangle_tangent_x.z * candidate3.x
                + triangle_tangent_y.z * candidate3.y
                + local_triangle_center.z;
        ...
        if use_side_i.simd_lt(zero_i).any() {
            ...
            dominant_edge_start = Vector3Wide::conditional_select(
                &bc_is_dominant_i,
                &triangle_b_world,
                &dominant_edge_start,
            );
            let mut dominant_edge_offset =
                Vector3Wide::conditional_select(&ab_is_dominant_i, &triangle_ab, &triangle_ca);
            dominant_edge_offset = Vector3Wide::conditional_select(
                &bc_is_dominant_i,
                &triangle_bc,
            ...
            let inverse_interpolation_span =
                Vector::<f32>::splat(1.0 / (upper_sin_sq - lower_sin_sq));
            let restrict_weight = ((dominant_edge_dot_hn_sq / interpolation_scale
                - interpolation_min)
            ...
            if use_side_edge_case_i.simd_lt(zero_i).any() {
                ...
                let numerator =
                    cyl_edge_to_dom_x * local_normal.z - cyl_edge_to_dom_z * local_normal.x;
                ...
                let projected_extent_offset = b.half_length
                    * dominant_edge_offset.y.abs()
                    * inverse_edge_offset_length_squared;
                ...
                let depth_base = (cyl_edge_to_dom_x * local_normal.x
                    + cyl_edge_to_dom_z * local_normal.z)
                ...
                local_offset_b0.x = use_side_edge_case
                    .select(dominant_edge_start.x + min_offset.x, local_offset_b0.x);
                local_offset_b0.y = use_side_edge_case
                    .select(dominant_edge_start.y + min_offset.y, local_offset_b0.y);
                local_offset_b0.z = use_side_edge_case
                    .select(dominant_edge_start.z + min_offset.z, local_offset_b0.z);
                local_offset_b1.x = use_side_edge_case
                    .select(dominant_edge_start.x + max_offset.x, local_offset_b1.x);
                local_offset_b1.y = use_side_edge_case
                    .select(dominant_edge_start.y + max_offset.y, local_offset_b1.y);
                local_offset_b1.z = use_side_edge_case
                    .select(dominant_edge_start.z + max_offset.z, local_offset_b1.z);
            }
            ...
            if use_side_triangle_face_i.simd_lt(zero_i).any() {
                ...
                let mut denominator_ab = min_to_max.x * edge_plane_ab.x
                    + min_to_max.y * edge_plane_ab.y
                    + min_to_max.z * edge_plane_ab.z;
                let mut denominator_bc = min_to_max.x * edge_plane_bc.x
                    + min_to_max.y * edge_plane_bc.y
                    + min_to_max.z * edge_plane_bc.z;
                let mut denominator_ca = min_to_max.x * edge_plane_ca.x
                    + min_to_max.y * edge_plane_ca.y
                    + min_to_max.z * edge_plane_ca.z;
                ...
                exit_ab =
                    ab_is_dominant.select(exit_ab * restrict_weight + unrestrict_weight, exit_ab);
                exit_bc =
                    bc_is_dominant.select(exit_bc * restrict_weight + unrestrict_weight, exit_bc);
                exit_ca =
                ...
                let mut vertex_fallback =
                    Vector3Wide::conditional_select(&use_a, &triangle_a_world, &triangle_c_world);
                vertex_fallback =
                ...
                local_offset_b0 = Vector3Wide::conditional_select(
                    &use_vertex_fallback_i,
                    &vertex_fallback,
                    &local_offset_b0,
                );

                let inverse_depth_denom =
                    one_f / (local_normal.x * local_normal.x + local_normal.z * local_normal.z);
                depth_t_min = use_side_triangle_face.select(
                    (local_normal.x * (closest_on_b.x - local_offset_b0.x)
                        + local_normal.z * (closest_on_b.z - local_offset_b0.z))
                        * inverse_depth_denom,
                    depth_t_min,
                );
                depth_t_max = use_side_triangle_face.select(
                    (local_normal.x * (closest_on_b.x - local_offset_b1.x)
                        + local_normal.z * (closest_on_b.z - local_offset_b1.z))
                        * inverse_depth_denom,
                    depth_t_max,
                );
            }
