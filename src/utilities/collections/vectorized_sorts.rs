        ...
        while i < element_count {
            ...
            for j in 0..i {
                ...
                let slot_precedes_value =
                    _mm256_castps_si256(_mm256_cmp_ps::<_CMP_LE_OS>(test_vector, values));
            ...
            for j in i..end_of_equality_testing {
                ...
                let slot_is_lesser =
                    _mm256_castps_si256(_mm256_cmp_ps::<_CMP_LT_OS>(test_vector, values));
...
mod arm_impl {
    ...
    pub unsafe fn vector_counting_sort_neon(
        ...
        debug_assert!(
            padded_keys.len() == padded_target_indices.len()
                && padded_keys.len().is_multiple_of(4)
                && padded_keys.len() >= element_count,
            "Preconditions not met: keys.len={}, indices.len={}, element_count={}",
            padded_keys.len(),
            padded_target_indices.len(),
        ...
        while i < element_count {
            ...
            for j in 0..i {
                ...
                let increment: int32x4_t =
                    vandq_s32(vreinterpretq_s32_u32(slot_precedes_value), one_mask);
            ...
            for j in i..end_of_equality_testing {
                ...
                // slot_precedes_value = lesser OR (equal AND index_lesser)
                let equal_and_index_lesser: uint32x4_t =
                    vandq_u32(slot_is_equal, slot_index_is_lesser);
                let slot_precedes_value: uint32x4_t =
                    vorrq_u32(slot_is_lesser, equal_and_index_lesser);
...
mod tests {
    ...
    fn test_counting_sort_basic() {
        ...
        let keys = vec![3.0f32, 1.0, 4.0, 1.0, 5.0, 9.0, 2.0, 6.0];
    ...
    fn test_counting_sort_scalar_matches_simd() {
        let keys = vec![5.0f32, 2.0, 8.0, 1.0, 9.0, 3.0, 7.0, 4.0];
